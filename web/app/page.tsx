...
function AttributeOverviewView({ attributes, state, onBack, onOpenAttribute }: { attributes: Attribute[]; state: WeekUpState; onBack: () => void; onOpenAttribute: (attributeId: string) => void }) {
  ...
  return <div className="view attribute-overview-view">
    ...
    <section className="attribute-overview-totals pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">ATTRIBUTE TOTALS</span><h2>各项属性总 XP</h2></div><strong>{overview.totalXp}<small>XP</small></strong></div><div className="overview-total-chart"><div className="overview-total-bars" style={{ "--attribute-count": totalRankedAttributes.length } as CSSProperties}>{totalRankedAttributes.map((item) => { const attribute = attributeById.get(item.attributeId); return <button type="button" key={item.attributeId} onClick={() => onOpenAttribute(item.attributeId)} title={`${item.name} ${item.totalXp} XP`}><b>{item.totalXp}</b><i><em style={{ height: `${item.totalXp === 0 ? 0 : Math.max(4, (item.totalXp / totalMaximum) * 100)}%`, background: attribute ? badgeColorValue(attribute.color) : "var(--violet)" }} /></i><small>{item.name}</small></button>; })}</div></div></section>
