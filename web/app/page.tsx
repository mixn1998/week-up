...
import type { AwarenessEntry, EmotionIntensity, EmotionType, MentalModelVersion } from "../lib/awareness";
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        <div className="page-wrap">
          ...
          {tab === "awareness" && <AwarenessView entries={weekUp.state.awarenessEntries} weeklyReviews={weekUp.state.weeklyEmotionReviews} monthlyReviews={weekUp.state.monthlyThoughtReviews} mentalModels={weekUp.state.mentalModelVersions} generatingIds={weekUp.generatingAwarenessIds} onUpdate={(entry, value, level, emotionType, intensity) => entry.kind === "thought" ? weekUp.dispatch({ type: "awareness.entry.update", id: entry.id, content: value }) : weekUp.dispatch({ type: "awareness.entry.update", id: entry.id, level: level ?? entry.level, emotionType, intensity, reason: value })} onRemove={(id) => weekUp.dispatch({ type: "awareness.entry.remove", id })} onRetryWeekly={(id) => weekUp.dispatch({ type: "awareness.weekly-analysis.retry", id })} onRetryMonthly={(version: MentalModelVersion) => weekUp.dispatch({ type: "awareness.monthly-analysis.retry", ...(version.sourceThoughtReviewId ? { thoughtReviewId: version.sourceThoughtReviewId } : {}), mentalModelVersionId: version.id })} />}
