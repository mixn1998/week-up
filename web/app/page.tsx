...
function WeekDashboard({ attributes, plans, planRecords, goals, settlements, initialRange, generatingHarvestIds, onRetryHarvest, onNewGoal, onEditGoal, onQuickAdd, onOpenCalendar, onOpenGrowth, onComplete, onEditPlan, onUndoPlan, onRemovePlan, onRescheduleOverdue }: { attributes: Attribute[]; plans: PlanItem[]; planRecords: readonly PlanRecord[]; goals: readonly GoalRecord[]; settlements: readonly SettlementRecord[]; initialRange?: DateRange; generatingHarvestIds: readonly string[]; onRetryHarvest: (id: string) => void; onNewGoal: () => void; onEditGoal: (goal: GoalRecord) => void; onQuickAdd: (goalIds?: readonly string[]) => void; onOpenCalendar: () => void; onOpenGrowth: () => void; onComplete: (id: string, segmentId?: string) => void; onEditPlan: (id: string) => void; onUndoPlan: (id: string, segmentId?: string) => void; onRemovePlan: (id: string) => void; onRescheduleOverdue: (id: string) => void }) {
  ...
  return <div className="view period-dashboard week-dashboard">
    ...
    <section className="week-route" aria-label="七日行动地图">{days.map((date, index) => { const dayPlans = periodPlans.filter((plan) => planDate(plan, recordMap, range) === date); const countedDayPlans = dayPlans.filter((plan) => plan.completed || !plan.overdue); const completed = countedDayPlans.filter((plan) => plan.completed).length; const lit = countedDayPlans.length > 0 && completed === countedDayPlans.length; return <button className={`day-tile${lit ? " is-lit" : ""}${date === currentLocalDate() ? " is-today" : ""}`} key={date} onClick={onOpenCalendar}><span>{["一", "二", "三", "四", "五", "六", "日"][index]}</span><b>{date.slice(8)}</b><i style={{ width: `${countedDayPlans.length ? Math.round((completed / countedDayPlans.length) * 100) : 0}%` }} /><small>{countedDayPlans.length ? `${completed}/${countedDayPlans.length} 完成` : dayPlans.length ? `${dayPlans.length} 项逾期` : "自由日"}</small></button>; })}</section>
