...
import { isLearningMoreCourseBundlePlan, isLearningMoreCourseComplete, isLearningMoreCoursePlan, isWeeklyRepeatDayLocked, takeVisibleGroupedRows } from "../lib/weekly-action-visibility";
...
import { selectUnconfiguredPlansForDate } from "../lib/unconfigured-time";
...
import type { AwarenessEntry, EmotionType, MentalModelVersion } from "../lib/awareness";
...
function WeeklyRepeatedPlanCard({ plans, dates, settledDates, attributes, readOnly, onComplete, onEdit, onUndo }: { plans: readonly PlanItem[]; dates: readonly string[]; settledDates: ReadonlySet<string>; attributes: Attribute[]; readOnly: boolean; onComplete: (id: string, segmentId?: string) => void; onEdit: (id: string) => void; onUndo: (id: string, segmentId?: string) => void }) {
  const first = plans[0]!;
  ...
  return <article className="weekly-repeat-card">
    ...
    <div className="weekly-repeat-days" aria-label={`${first.title}本周重复行动`}>
      {dates.map((date, index) => {
        ...
        const locked = isWeeklyRepeatDayLocked(plan, date, settledDates, readOnly);
        ...
        return <span className={`weekly-repeat-day${plan.completed ? " is-complete" : ""}`} key={date}>
          ...
          {!locked && <button type="button" className="weekly-repeat-day__edit" onClick={() => onEdit(plan.id)}>编辑</button>}
        </span>;
      })}
    </div>
  </article>;
...
function QuickWeightEntry({ onSave }: { onSave: (value: number) => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (parsed >= 20 && parsed <= 300) onSave(Math.round(parsed * 10) / 10);
  };
  return <form className="weight-form weight-form--quick" onSubmit={submit}>
    <label htmlFor="quick-weight">今日体重</label>
    <div><input id="quick-weight" aria-label="今日体重快捷输入" type="number" min="20" max="300" step="0.1" value={value} onChange={(event) => setValue(event.target.value)} placeholder="--.-" /><span>kg</span></div>
    <button className="pixel-button pixel-button--pink" type="submit" disabled={Number(value) < 20 || Number(value) > 300}>保存记录</button>
  </form>;
}
...
function TodayView({
  ...
  const hasTodayWeight = weights.some((entry) => entry.date === currentLocalDate());
      ...
      <div className="today-grid">
        <section className="pixel-card schedule-panel">
          ...
          <div className={`timeline${todayPlans.length > 8 ? " timeline--dense" : ""}`}>
            ...
            {completedPlans.length > 0 && <section className="plan-group completed-group"><button className="plan-group__toggle" type="button" aria-expanded={completedOpen} onClick={() => setCompletedOpen((current) => !current)}><span>已完成 {completedPlans.length} 项</span><b>{completedOpen ? "收起" : "展开"}</b></button>{completedOpen && completedPlans.map((plan) => <PlanRow key={plan.id} plan={plan} attributes={attributes} onComplete={onComplete} onEdit={onEdit} onUndo={onUndo} onRemove={onRemove} />)}</section>}
          </div>
        </section>
...
function PeriodFacts({ period, range, plans, attributes, settlement, generatingHarvestIds, onRetryHarvest, afterGrowth }: { period: "week" | "month"; range: DateRange; plans: readonly PlanItem[]; attributes: readonly Attribute[]; settlement?: SettlementRecord; generatingHarvestIds: readonly string[]; onRetryHarvest: (id: string) => void; afterGrowth?: ReactNode }) {
  ...
  const completedCount = settlement ? settlement.completedPlanIds.length : completedPlans.length;
  const missedPlans = period === "week"
    ? selectPeriodOverduePlans(plans, range.startDate, range.endDate, settlement?.overduePlanIds)
    : settlement
      ? plans.filter((plan) => settlement.incompletePlanIds.includes(plan.id))
...
function WeekDashboard({ attributes, plans, planRecords, goals, dailySettlements, settlements, initialRange, generatingHarvestIds, onRetryHarvest, onNewGoal, onEditGoal, onQuickAdd, onOpenCalendar, onOpenGrowth, onComplete, onEditPlan, onUndoPlan, onRemovePlan, onRescheduleOverdue }: { attributes: Attribute[]; plans: PlanItem[]; planRecords: readonly PlanRecord[]; goals: readonly GoalRecord[]; dailySettlements: WeekUpState["dailySettlements"]; settlements: readonly SettlementRecord[]; initialRange?: DateRange; generatingHarvestIds: readonly string[]; onRetryHarvest: (id: string) => void; onNewGoal: () => void; onEditGoal: (goal: GoalRecord) => void; onQuickAdd: (goalIds?: readonly string[]) => void; onOpenCalendar: () => void; onOpenGrowth: () => void; onComplete: (id: string, segmentId?: string) => void; onEditPlan: (id: string) => void; onUndoPlan: (id: string, segmentId?: string) => void; onRemovePlan: (id: string) => void; onRescheduleOverdue: (id: string) => void }) {
  ...
  const frozenDailyDates = new Set(dailySettlements.map((settlement) => settlement.localDate));
  ...
  const renderActionEntry = (entry: WeeklyActionEntry) => entry.kind === "single"
    ...
    : entry.kind === "course"
      ...
      : <WeeklyRepeatedPlanCard key={entry.id} plans={entry.plans} dates={days} settledDates={frozenDailyDates} attributes={attributes} readOnly={Boolean(selectedSettlement)} onComplete={onComplete} onEdit={onEditPlan} onUndo={onUndoPlan} />;
  ...
  return <div className="view period-dashboard week-dashboard">
    ...
    <PeriodFacts period="week" range={range} plans={periodPlans} attributes={attributes} settlement={selectedSettlement} generatingHarvestIds={generatingHarvestIds} onRetryHarvest={onRetryHarvest} />
  </div>;
}
...
function MonthDashboard({ attributes, plans, planRecords, goals, projects, projectCategories, dailySettlements, settlements, weights, generatingHarvestIds, onRetryHarvest, onNewGoal, onEditGoal, onOpenWeek, onOpenCalendar, onOpenWeight }: { attributes: Attribute[]; plans: PlanItem[]; planRecords: readonly PlanRecord[]; goals: readonly GoalRecord[]; projects: readonly ProjectRecord[]; projectCategories: readonly AttributeCategoryRecord[]; dailySettlements: WeekUpState["dailySettlements"]; settlements: readonly SettlementRecord[]; weights: WeightEntry[]; generatingHarvestIds: readonly string[]; onRetryHarvest: (id: string) => void; onNewGoal: () => void; onEditGoal: (goal: GoalRecord) => void; onOpenWeek: (range?: DateRange) => void; onOpenCalendar: () => void; onOpenWeight: () => void }) {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  ...
  return <div className="view period-dashboard month-dashboard">
    ...
    <section className="month-week-route" style={{ "--month-week-count": slices.length } as CSSProperties}>{slices.map((slice, index) => {
      const slicePlans = periodPlans.filter((plan) => { const date = planDate(plan, recordMap, range); return date !== undefined && date >= slice.startDate && date <= slice.endDate; });
      const settledWeek = settlements.find((item) => item.period === "week" && item.startDate <= slice.startDate && item.endDate >= slice.endDate);
      const frozenDays = dailySettlements.filter((item) => item.localDate >= slice.startDate && item.localDate <= slice.endDate);
      const frozenSliceIds = new Set(frozenDays.flatMap((item) => item.planIds));
      const fullSettledWeek = settledWeek?.startDate === slice.startDate && settledWeek.endDate === slice.endDate;
      const frozen = settledWeek ? {
        planIds: fullSettledWeek ? settledWeek.planIds : settledWeek.planIds.filter((id) => frozenSliceIds.has(id)),
        completedPlanIds: fullSettledWeek ? settledWeek.completedPlanIds : settledWeek.completedPlanIds.filter((id) => frozenSliceIds.has(id)),
      } : undefined;
      const summary = summarizeWeekRouteDay(slicePlans, frozen);
      return <button key={slice.startDate} onClick={() => onOpenWeek(weekRangeFor(slice.startDate))}><span>W{index + 1}</span><b>{slice.startDate.slice(5)}—{slice.endDate.slice(5)}</b><i><em style={{ width: `${summary.progress}%` }} /></i><small>{summary.total ? summary.label : "等待行动"}</small></button>;
    })}</section>
...
function CalendarView({ plans, unconfiguredPlans, untimedCompletionPlans, settledDates, initialMode, content, onEditPlan }: { plans: PlanItem[]; unconfiguredPlans: PlanItem[]; untimedCompletionPlans: PlanItem[]; settledDates: readonly string[]; initialMode: "week" | "month"; content: "timeline" | "schedule"; onEditPlan: (id: string) => void }) {
  ...
  const settledDateSet = useMemo(() => new Set(settledDates), [settledDates]);
  ...
  const unconfiguredPlansForDate = (dateKey: string) => selectUnconfiguredPlansForDate(
    dateKey,
    unconfiguredPlans,
    untimedCompletionPlans,
    settledDateSet,
  )
  ...
  const selectedUnscheduled = selectedDate && mode === "week" ? unconfiguredPlansForDate(selectedDate) : [];
    ...
    <div className="view">
      ...
      {mode === "week" ? <section className="calendar-card pixel-card">
        ...
        <div className="unscheduled-dock">
          <div className="unscheduled-dock__label"><span>{content === "timeline" ? "UNTIMED" : "POCKET"}</span><b>{content === "timeline" ? "未配置时间" : "待安排"}</b></div>
          {dayDates.map((date, dayIndex) => {
            const count = unconfiguredPlansForDate(date.key).length;
            const countDescription = content === "timeline" ? "项未配置时间任务" : "项待安排计划";
            return <button className={`${dayIndex === todayIndex ? "is-today" : ""}${count > 0 ? " has-items" : ""}`} key={date.key} disabled={count === 0} onClick={() => setSelectedDate(date.key)} aria-label={`${date.weekday}有${count}${countDescription}`}>{count > 0 ? <><strong>+{count}</strong><small>{content === "timeline" ? "展开未配置任务" : "展开待安排"}</small></> : <span>—</span>}</button>;
          })}
        </div>
        ...
        <div className="mobile-agenda">{dayDates.map((date, dayIndex) => {
          ...
          const unscheduledPlans = unconfiguredPlansForDate(date.key);
          ...
          return <section className="agenda-day" key={date.key}><header><b>{date.weekday} {date.day}</b><span>{timedPlans.length} {content === "timeline" ? "已记录" : "已排"} · {unscheduledPlans.length} {content === "timeline" ? "未配置时间" : "待安排"}</span></header>{unscheduledPlans.length > 0 && <button className="agenda-unscheduled" onClick={() => setSelectedDate(date.key)}><strong>+{unscheduledPlans.length}</strong><span>{content === "timeline" ? "展开未配置时间完成记录" : "展开待安排计划"}</span><em>→</em></button>}{shown.map((plan) => <div className="agenda-row" key={plan.id}>{content === "timeline" ? <span className="agenda-row__time">{plan.start}—{plan.end}</span> : <button className="agenda-row__time" onClick={() => onEditPlan(plan.calendarSourceId ?? plan.id)}>{plan.start}—{plan.end}</button>}<span>{plan.title}</span><small>{plan.completed ? "已完成" : plan.source === "learning-more" ? "LM 同步" : plan.category}</small></div>)}{dayPlans.length === 0 && <p>{content === "timeline" ? "没有完成记录" : "没有安排"}</p>}{timedPlans.length > 6 && <button className="load-more" onClick={() => setExpandedDays((current) => expanded ? current.filter((item) => item !== dayIndex) : [...current, dayIndex])}>{expanded ? "收起" : `再显示 ${timedPlans.length - 6} 项`}</button>}</section>;
        })}</div>
      ...
      {selectedDate !== null && <div className="agenda-drawer pixel-card"><div className="section-heading"><div><span className="eyebrow">{mode === "month" ? "DONE TODAY" : content === "timeline" ? "COMPLETION FACTS" : "DAY AGENDA"}</span><h2>{selectedDateLabel} · {mode === "month" || content === "timeline" ? "完成记录" : "计划口袋"}</h2></div><button className="modal-close" onClick={() => setSelectedDate(null)}>×</button></div>{selectedUnscheduled.length > 0 || selectedTimed.length > 0 ? <>{mode === "week" && selectedUnscheduled.length > 0 && <section className="drawer-plan-group drawer-plan-group--unscheduled"><div className="drawer-plan-group__title"><b>{content === "timeline" ? "未配置时间任务" : "待安排"}</b><span>{selectedUnscheduled.length} 项 · 点击时间票券直接设置</span></div>{selectedUnscheduled.map((plan) => <CalendarDrawerPlan key={plan.id} plan={plan} mode={mode} onEditPlan={onEditPlan} />)}</section>}{selectedTimed.length > 0 && <section className="drawer-plan-group"><div className="drawer-plan-group__title"><b>{mode === "month" ? "完成内容" : content === "timeline" ? "实际执行" : "已安排"}</b><span>{selectedTimed.length} 项{content === "schedule" ? " · 点击时间可再次修改" : ""}</span></div>{selectedTimed.map((plan) => <CalendarDrawerPlan key={plan.id} plan={plan} mode={mode} readOnly={content === "timeline"} onEditPlan={onEditPlan} />)}</section>}</> : <div className="mini-empty">{mode === "month" || content === "timeline" ? "这一天还没有完成记录。" : "这一天还没有计划。"}</div>}</div>}
    </div>
  );
}
...
function AttributeOverviewView({ attributes, state, onBack, onOpenAttribute }: { attributes: Attribute[]; state: WeekUpState; onBack: () => void; onOpenAttribute: (attributeId: string) => void }) {
  ...
  const categoryTotal = Math.max(1, overview.categories.reduce((sum, category) => sum + category.totalXp, 0));
  let categoryCursor = 0;
  const categoryGradient = overview.categories.length === 0 || overview.totalXp === 0
    ? "#eee8f0"
    : `conic-gradient(${overview.categories.map((category) => {
      const start = categoryCursor;
      categoryCursor += (category.totalXp / categoryTotal) * 100;
  ...
  return <div className="view attribute-overview-view">
    ...
    <div className="attribute-analytics-grid">
      <section className="attribute-analytics-panel pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">ATTRIBUTE MIX</span><h2>属性分类构成</h2></div><strong>{overview.categories.length}<small>类</small></strong></div><div className="overview-category-donut"><div className="overview-category-donut__ring" style={{ background: categoryGradient }}><span><b>{overview.totalXp}</b><small>总 XP</small></span></div><div className="overview-category-donut__legend">{overview.categories.map((category) => <div key={category.category}><i style={{ background: colorForCategory(category.category) }} /><b>{category.category}</b><span>{category.attributeCount} 项</span><strong>{category.totalXp} XP</strong><small>{Math.round((category.totalXp / categoryTotal) * 100)}%</small></div>)}</div></div></section>
      <section className="attribute-analytics-panel pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">LEVEL DISTRIBUTION</span><h2>徽章等级分布</h2></div><strong>{overview.attributeCount}<small>枚</small></strong></div><div className="overview-level-distribution">{overview.levelDistribution.map((level) => <div key={level.label}><b>{level.label}</b><i><em style={{ width: `${level.count === 0 ? 0 : Math.max(6, (level.count / levelMaximum) * 100)}%` }} /></i><strong>{level.count} 枚</strong><small>{overview.attributeCount === 0 ? 0 : Math.round((level.count / overview.attributeCount) * 100)}%</small></div>)}</div></section>
    </div>
    <section className="attribute-overview-totals pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">ATTRIBUTE TOTALS</span><h2>各项属性总 XP</h2></div><strong>{overview.totalXp}<small>XP</small></strong></div><div className="overview-total-chart"><div className="overview-total-bars" style={{ "--attribute-count": totalChartSlotCount, "--chart-width": totalChartWidth } as CSSProperties}>{totalRankedAttributes.map((item) => { const attribute = attributeById.get(item.attributeId); const barHeight = item.totalXp === 0 ? 0 : Math.max(4, (item.totalXp / totalMaximum) * 100); return <button type="button" key={item.attributeId} onClick={() => onOpenAttribute(item.attributeId)} title={`${item.name} ${item.totalXp} XP`}><i style={{ "--bar-height": `${barHeight}%` } as CSSProperties}><b>{item.totalXp}</b><em style={{ background: attribute ? badgeColorValue(attribute.color) : "var(--violet)" }} /></i><small>{item.name}</small></button>; })}</div></div></section>
    <section className="attribute-overview-table pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">ALL ATTRIBUTES</span><h2>全属性明细</h2></div><strong>本月 <b>+{overview.monthGain} XP</b></strong></div><div><header><span>属性</span><span>分类</span><span>等级</span><span>当前 XP</span><span>近 30 日</span></header>{visibleDetails.map((item) => { const attribute = attributeById.get(item.attributeId); return <button type="button" key={item.attributeId} onClick={() => onOpenAttribute(item.attributeId)}><span><i style={{ background: attribute ? badgeColorValue(attribute.color) : "var(--violet)" }} />{item.name}</span><span>{item.category}</span><span>LV.{item.level}</span><strong>{item.totalXp}</strong><em>+{item.thirtyDayGain}</em></button>; })}</div>{overview.attributes.length > 8 && <button className="attribute-overview-table__toggle" type="button" onClick={() => setDetailsOpen((current) => !current)}>{detailsOpen ? "收起属性明细 ↑" : `展开其余 ${overview.attributes.length - 8} 项 ↓`}</button>}</section>
  </div>;
}
...
function AttributeAnalyticsView({ attribute, state, onBack }: { attribute: Attribute; state: WeekUpState; onBack: () => void }) {
  ...
  const comparisonMatch = analytics.thirtyDay.comparisonLabel.match(/^(.+?)\s*(XP|%)$/);
  const comparisonMetric = {
    value: comparisonMatch?.[1] ?? analytics.thirtyDay.comparisonLabel,
    unit: comparisonMatch?.[2] ?? "",
  };
  ...
  return (
    <div className="view attribute-analytics-view">
      ...
      <div className="attribute-analytics-grid">
        <section className="attribute-analytics-panel pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">30 DAY TREND</span><h2>30 日累计趋势</h2></div><strong>{comparisonMetric.value}{comparisonMetric.unit && <small>{comparisonMetric.unit}</small>}</strong></div><svg className="attribute-trend-chart" viewBox="0 0 640 180" role="img" aria-label={`${attribute.name}最近30日累计XP趋势`}><line x1="34" y1="26" x2="606" y2="26" /><line x1="34" y1="67" x2="606" y2="67" /><line x1="34" y1="108" x2="606" y2="108" /><line x1="34" y1="150" x2="606" y2="150" /><polyline points={trendPoints} /><text x="34" y="173">{analytics.thirtyDay.points[0]?.localDate.slice(5)}</text><text x="570" y="173">{analytics.thirtyDay.points.at(-1)?.localDate.slice(5)}</text></svg></section>
        <section className="attribute-analytics-panel pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">WEEKLY DELTA</span><h2>每周增量</h2></div><strong>4<small>周</small></strong></div><div className="attribute-week-bars">{analytics.weeklyGains.map((week, index) => <div className={index === analytics.weeklyGains.length - 1 ? "is-current" : ""} key={week.startDate}><b>+{week.amount} XP</b><i><span style={{ height: `${week.amount === 0 ? 0 : Math.max(8, (week.amount / weeklyMaximum) * 100)}%` }} /></i><small>{index === analytics.weeklyGains.length - 1 ? "本周" : week.startDate.slice(5)}</small></div>)}</div></section>
        <section className="attribute-analytics-panel pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">CATEGORY SOURCES</span><h2>来源构成</h2></div><strong>{analytics.totalXp}<small>XP</small></strong></div>{analytics.categoryGains.length === 0 ? <div className="mini-empty">还没有可分析的 XP 来源。</div> : <div className="attribute-source-composition"><div className="attribute-source-donut" style={{ background: categoryGradient }}><span>{analytics.totalXp}<small>XP</small></span></div><div>{analytics.categoryGains.map((category) => <p key={category.category}><i style={{ background: colorForCategory(category.category) }} /><b>{category.category}</b><span>{category.amount} XP · {Math.round((category.amount / categoryTotal) * 100)}%</span></p>)}</div></div>}</section>
        <section className="attribute-analytics-panel pixel-card"><div className="analytics-panel-heading"><div><span className="eyebrow">ACTIVITY RHYTHM</span><h2>增长节奏</h2></div><strong>{analytics.activeDates.length}<small>个活跃日</small></strong></div><div className="attribute-activity-labels"><span>30 天前</span><span>今天</span></div><div className="attribute-activity-grid">{analytics.thirtyDay.points.map((point) => <i key={point.localDate} title={`${point.localDate} +${point.gainedXp} XP`} style={{ "--activity-level": point.gainedXp === 0 ? 0 : Math.max(0.2, point.gainedXp / activityMaximum) } as CSSProperties} className={point.gainedXp > 0 ? "is-active" : ""} />)}</div><div className="attribute-activity-foot"><span>颜色越深，单日获得 XP 越多</span><span>最长连续增长 <b>{analytics.longestStreak} 天</b></span></div></section>
      </div>
...
function GrowthView({ attributes, skillbooks, goals, state }: { attributes: Attribute[]; skillbooks: readonly SkillbookRecord[]; goals: readonly GoalRecord[]; state: WeekUpState }) {
  ...
  return (
    <div className="view">
      <div className="page-title"><div><span className="eyebrow">PIXEL ATLAS</span><h1>成就图鉴</h1></div></div>
      ...
      {section === "badges" ? <>{attributes.length > 0 && <div className="category-summary-row"><div className="category-summary" aria-label="按属性类别筛选"><button className={`category-summary__item category-summary__item--all${activeCategory === "all" ? " active" : ""}`} type="button" aria-pressed={activeCategory === "all"} onClick={() => setSelectedCategory("all")}><i />全部<b>{attributes.length}</b></button>{categories.map((category) => <button className={`category-summary__item${activeCategory === category ? " active" : ""}`} type="button" aria-pressed={activeCategory === category} style={{ "--category-accent": colorForCategory(category) } as CSSProperties} key={category} onClick={() => setSelectedCategory(category)}><i />{category}<b>{attributes.filter((attribute) => (attribute.category ?? "未分类") === category).length}</b></button>)}</div>{analyticsEnabled && section === "badges" && <button className="attribute-overview-entry" type="button" onClick={() => setOverviewOpen(true)}><span>▦</span><b>属性总览</b><small>全属性分析 →</small></button>}</div>}
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <button className="brand" onClick={() => setTab("today")} aria-label="Week UP 首页"><img className="brand-mark" src="/week-up-logo.svg" alt="" /><span><b>WEEK</b><strong>UP!</strong></span></button>
      ...
      <main className="main-area">
        ...
        <div className="page-wrap">
          ...
          {tab === "awareness" && <AwarenessView entries={weekUp.state.awarenessEntries} weeklyReviews={weekUp.state.weeklyEmotionReviews} monthlyReviews={weekUp.state.monthlyThoughtReviews} mentalModels={weekUp.state.mentalModelVersions} generatingIds={weekUp.generatingAwarenessIds} onUpdate={(entry, value, level, emotionType) => entry.kind === "thought" ? weekUp.dispatch({ type: "awareness.entry.update", id: entry.id, content: value }) : weekUp.dispatch({ type: "awareness.entry.update", id: entry.id, level: level ?? entry.level, emotionType, reason: value })} onRemove={(id) => weekUp.dispatch({ type: "awareness.entry.remove", id })} onRetryWeekly={(id) => weekUp.dispatch({ type: "awareness.weekly-analysis.retry", id })} onRetryMonthly={(version: MentalModelVersion) => weekUp.dispatch({ type: "awareness.monthly-analysis.retry", ...(version.sourceThoughtReviewId ? { thoughtReviewId: version.sourceThoughtReviewId } : {}), mentalModelVersionId: version.id })} />}
