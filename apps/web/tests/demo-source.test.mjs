...
import { levelFromTotalXp, movingAverage } from "../src/lib/demo-model.ts";
import { clusterCalendarPlans, projectCalendarCluster, rescheduleRange } from "../src/lib/calendar-layout.ts";
import { CATEGORY_PALETTE, colorForCategory } from "../src/lib/category-palette.ts";
...
test("keeps Today's completed plans expanded by default and manually collapsible", async () => {
  const page = await readFile(new URL("../src/app/page.tsx", import.meta.url), "utf8");
  const todayView = page.match(/function TodayView\([\s\S]*?\nfunction GoalsView/)?.[0] ?? "";

  assert.match(todayView, /const \[completedOpen, setCompletedOpen\] = useState\(true\);/);
  assert.match(todayView, /\{completedOpen && completedPlans\.map/);
  assert.doesNotMatch(todayView, /completedOpen \|\| todayPlans\.length <= 8/);
});

test("locks frozen dates only inside exact-daily repeated action cards", async () => {
  const page = await readFile(new URL("../src/app/page.tsx", import.meta.url), "utf8");
  const repeatedCard = page.match(/function WeeklyRepeatedPlanCard\([\s\S]*?\nfunction WeightChart/)?.[0] ?? "";

  assert.match(repeatedCard, /isWeeklyRepeatDayLocked\(plan, date, settledDates, readOnly\)/);
  assert.match(repeatedCard, /\{!locked && <button type="button" className="weekly-repeat-day__edit"/);
  assert.match(page, /settledDates=\{frozenDailyDates\}/);
});
...
test("adds sparse self-awareness capture and three independent archive tabs", async () => {
  const [page, awareness, awarenessDomain] = await Promise.all([
    ...
    readFile(new URL("../src/app/awareness-view.tsx", import.meta.url), "utf8"),
    readFile(new URL("../src/lib/awareness.ts", import.meta.url), "utf8"),
  ]);
  ...
  assert.doesNotMatch(awareness, /空白日期不补值/);
  assert.match(awareness, /随新记录更新/);
...
test("mental model view renders one current radar without version comparison cards", async () => {
  const [awareness, radar] = await Promise.all([
    readFile(new URL("../src/app/awareness-view.tsx", import.meta.url), "utf8"),
    readFile(new URL("../src/app/mental-model-radar.tsx", import.meta.url), "utf8"),
  ]);
  ...
  assert.doesNotMatch(radar, /mental-radar__value/);
  assert.doesNotMatch(radar, /数值表示记录证据强度/);
  assert.match(radar, /核心看法/);
  assert.match(radar, /常见判断/);
  assert.match(radar, /行动方式/);
  assert.match(radar, /setSelectedDimension/);
  assert.match(radar, /aria-live="polite"/);
  assert.doesNotMatch(radar, /MentalModelDimensionList/);
  assert.doesNotMatch(radar, /八维心智结构/);
});

test("self-awareness reviews use structured trigger analysis and non-chronological monthly signals", async () => {
  const [awareness, css] = await Promise.all([
    readFile(new URL("../src/app/awareness-view.tsx", import.meta.url), "utf8"),
    readFile(new URL("../src/app/globals.css", import.meta.url), "utf8"),
  ]);
  for (const label of ["可能的触发机制", "有待验证", "本月思想信号", "反复出现的主线", "新出现的线索", "有待观察"]) {
    assert.match(awareness, new RegExp(label));
  }
  assert.doesNotMatch(awareness, /月初更常出现|月末逐渐形成/);
  assert.match(awareness, /心智模型 · 增量证据/);
  assert.match(css, /\.awareness-trigger-chain/);
  assert.match(css, /\.awareness-thought-signals/);
});

test("keeps failed awareness analysis states on one compact title-action row", async () => {
  const [awareness, css] = await Promise.all([
    readFile(new URL("../src/app/awareness-view.tsx", import.meta.url), "utf8"),
    readFile(new URL("../src/app/globals.css", import.meta.url), "utf8"),
  ]);
  const analysisStatus = awareness.slice(awareness.indexOf("function AnalysisStatus"), awareness.indexOf("function ThoughtReviewCard"));
  assert.doesNotMatch(analysisStatus, /<small>/);
  assert.match(css, /\.awareness-analysis-state \{[^}]*grid-template-columns: minmax\(0,1fr\) auto;[^}]*align-items: center;/);
  assert.match(css, /\.awareness-analysis-state \.pixel-button \{[^}]*justify-self: end;[^}]*margin-top: 0;/);
});
...
test("uses one shared pixel logo across browser, desktop, mobile, and milestone surfaces", async () => {
  const [page, milestoneSample, index, logo] = await Promise.all([
    readFile(new URL("../src/app/page.tsx", import.meta.url), "utf8"),
    readFile(new URL("../src/app/milestone-ui-sample.tsx", import.meta.url), "utf8"),
...
test("styles only rescheduled overdue labels with a strong red background", async () => {
  const css = await readFile(new URL("../src/app/globals.css", import.meta.url), "utf8");
  const carriedRule = css.match(/\.overdue-chip--carried \{([^}]*)\}/)?.[1] ?? "";
  assert.match(css, /\.overdue-chip \{[^}]*background: #f1e8f4;[^}]*color: #a61b50 !important;/);
  assert.match(css, /\.overdue-chip--carried \{[^}]*background: var\(--coral\);[^}]*color: white !important;/);
  assert.match(css, /\.dashboard-action-list \.overdue-chip--carried \{[^}]*background: var\(--coral\);[^}]*color: white !important;/);
  assert.doesNotMatch(carriedRule, /box-shadow:\s*inset/);
});
