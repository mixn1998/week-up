...
import { createEmptyWeekUpState, dispatchWeekUp, migrateWeekUpState } from "../src/lib/week-up-domain.ts";
import { dueDailySettlementCommands, dueSettlementCommands } from "../src/lib/settlement-scheduler.ts";
...
test("collapsed legacy dimensions rebuild from distinct relevant models", () => {
  const stableKeys = [
    "internal_agency_and_response",
    "relationship_as_dynamic_co_creation",
    "optionality_power_and_negotiation",
    "process_rhythm_and_practice",
    "structural_semantic_learning",
    "long_term_nonzero_sum_value",
    "vitality_with_restraint",
    "contextual_dynamic_systems",
  ];
  const models = stableKeys.map((stableKey, index) => ({
    stableKey,
    name: `模型 ${index + 1}`,
    summary: `维度 ${index + 1} 的独立结论`,
    triggers: [],
    assumptions: [`判断 ${index + 1}`],
    defaultResponses: [],
    currentStrategies: [`行动 ${index + 1}`],
    supportingEntryIds: [],
    counterEvidenceEntryIds: [],
    confidence: "high",
    changeType: "new",
  }));
  const collapsed = MENTAL_MODEL_DIMENSIONS.map((definition) => ({
    dimension: definition.key,
    strength: 98,
    confidence: "high",
    summary: "重复结论",
    defaultJudgments: ["重复判断"],
    currentStrategies: ["重复行动"],
    supportingModelKeys: stableKeys,
    changeDirection: "new",
  }));
  const rebuilt = completeMentalModelDimensionProfile(models, collapsed);
  assert.equal(new Set(rebuilt.map((item) => item.summary)).size, 8);
  assert.ok(rebuilt.every((item) => item.supportingModelKeys.length <= 2));
});
