...
import {
  ...
  migrateWeekUpState,
  totalXpForAttribute,
} from "../src/lib/week-up-domain.ts";
import { HttpWeekUpRepository, MemoryWeekUpRepository } from "../src/lib/week-up-repository.ts";
import { createWeekUpStore } from "../src/lib/week-up-store.ts";
...
test("reschedules an overdue plan without changing its id and grants rewards only once", () => {
  const h = harness("2026-07-20T08:00:00+08:00");
  let state = addAttribute(h, createEmptyWeekUpState());
  state = addPlan(h, state, state.attributes[0].id, {
    title: "重新出发",
  ...
  const originalSegment = state.plans[0].timeSegments[0];
  ...
  assert.equal(state.plans[0].startAt, "2026-07-20T18:00:00+08:00");
  assert.deepEqual(state.plans[0].scheduleHistory, [{
    startAt: "2026-07-19T09:00:00+08:00",
    endAt: "2026-07-19T10:00:00+08:00",
    timeStatus: "scheduled",
    timeSegments: [originalSegment],
    rescheduledAt: "2026-07-20T08:00:00+08:00",
  }]);
  h.setNow("2026-07-21T08:00:00+08:00");
  state = h.run(state, { type: "plan.overdue.reschedule", id: originalId, startAt: "2026-07-21T18:00:00+08:00", endAt: "2026-07-21T19:00:00+08:00" });
  assert.equal(state.plans.length, 1);
  assert.equal(state.plans[0].id, originalId);
  assert.equal(state.plans[0].scheduleHistory.length, 2);
  state = h.run(state, { type: "plan.complete", id: originalId, completedAt: "2026-07-21T19:00:00+08:00" });
...
test("can carry an overdue plan into a new date without assigning a time", () => {
  ...
  assert.equal(state.plans[0].timeStatus, "unscheduled");
  assert.deepEqual(state.plans[0].timeSegments, []);
  assert.equal(state.plans[0].startAt.slice(0, 10), "2026-07-21");
});
...
test("freezes weekly plan, completion, and overdue facts before later queue changes", () => {
  ...
  const rescheduledId = frozen.overduePlanIds[0];
  h.setNow("2026-07-27T10:00:00+08:00");
  state = h.run(state, { type: "plan.complete", id: rescheduledId, completedAt: "2026-07-27T10:00:00+08:00" });
...
test("collapses legacy overdue reschedule chains onto the original plan id", () => {
  const h = harness("2026-07-20T08:00:00+08:00");
  let state = addAttribute(h, createEmptyWeekUpState());
  state = addPlan(h, state, state.attributes[0].id, {
    title: "旧版重排",
  ...
  const carriedId = "legacy-carried-plan";
  const carried = {
    ...original,
    id: carriedId,
    startAt: "2026-07-20T18:00:00+08:00",
    endAt: "2026-07-20T19:00:00+08:00",
    timeSegments: [{ id: `${carriedId}:segment:0`, startAt: "2026-07-20T18:00:00+08:00", endAt: "2026-07-20T19:00:00+08:00" }],
    overdueSourcePlanId: original.id,
    createdAt: "2026-07-20T08:00:00+08:00",
    updatedAt: "2026-07-20T08:00:00+08:00",
  };
  state = {
    ...state,
    plans: [{ ...original, overdueRescheduledPlanId: carriedId }, carried],
  };
  state = h.run(state, { type: "plan.complete", id: carriedId, completedAt: "2026-07-20T19:00:00+08:00" });
  const transactionCount = state.xpTransactions.length;
  const migrated = migrateWeekUpState({
    ...
    dailySettlements: [{
      ...
      settledAt: "2026-07-20T00:30:00+08:00",
      planIds: [original.id, carriedId],
      completedPlanIds: [carriedId],
    }],
    settlements: [{
      id: "legacy-week",
      period: "week",
      ...
      endDate: "2026-07-25",
      generatedAt: "2026-07-26T00:30:00+08:00",
      planIds: [original.id, carriedId],
      completedPlanIds: [carriedId],
      overduePlanIds: [original.id],
      incompletePlanIds: [],
      attributeGains: { [state.attributes[0].id]: 2 },
      harvest: { status: "pending" },
    }],
  });

  assert.equal(migrated.plans.length, 1);
  assert.equal(migrated.plans[0].id, original.id);
  assert.equal(migrated.plans[0].startAt, carried.startAt);
  assert.equal(migrated.plans[0].scheduleHistory.length, 1);
  assert.deepEqual(migrated.completionFacts.map((fact) => fact.planId), [original.id]);
  assert.equal(migrated.xpTransactions.length, transactionCount);
  assert.equal(totalXpForAttribute(migrated, state.attributes[0].id), 2);
  assert.deepEqual(migrated.dailySettlements[0].planIds, [original.id]);
  assert.deepEqual(migrated.dailySettlements[0].completedPlanIds, [original.id]);
  assert.deepEqual(migrated.settlements[0].planIds, [original.id]);
  assert.deepEqual(migrated.settlements[0].completedPlanIds, [original.id]);
  assert.deepEqual(migrated.settlements[0].overduePlanIds, [original.id]);
});
