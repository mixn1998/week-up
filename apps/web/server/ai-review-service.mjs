...
export function buildAwarenessPrompt(facts) {
  const shared = [
    ...
    "情绪事件中的 emotionType 表示低落、焦虑、愤怒、愉悦、激动或复杂；“强烈”只是用户选择记录该事件的门槛，不是一个可统计等级。",
  ...
  if (facts.kind === "weekly-emotion") {
    return [
      ...shared,
      "分析目标是从已记录事件中提出可核验的触发机制：事件 → 当时的理解 → 被触及的需要或价值 → 情绪反应。所有机制都必须写成“可能的解释”，不得诊断人格，也不得把单次事件概括成稳定模式。",
      "alternativeExplanations 至少给出一种合理的其他解释；pendingValidations 使用“有待验证”的问法，帮助用户在下一次类似事件中核对，而不是布置任务。",
      "mentalModelSignals 只记录可能影响心智模型的新证据。仅凭单次事件可以标记线索，但不得声称已经形成稳定心智模式。",
      "输出结构：",
      JSON.stringify({
        kind: "weekly-emotion",
        ...
        emotion: {
          ...
          triggerChains: [{
            eventSummary: "发生了什么",
            interpretation: "当时可能如何理解这件事",
            underlyingNeeds: ["被触及的需要或价值"],
            emotionalResponse: "情绪如何出现或变化",
            possibleMechanism: "为什么这种理解可能触发这种情绪",
            evidenceEntryIds: ["真实 entryId"],
          }],
          alternativeExplanations: [{ summary: "另一种合理解释", evidenceEntryIds: ["真实 entryId"] }],
          pendingValidations: [{ question: "有待验证的问题", evidenceEntryIds: ["真实 entryId"] }],
          mentalModelSignals: [{
            dimension: "self|relationships|power|action|learning|values|vitality|world",
            summary: "可能进入心智模型的新证据",
            evidenceEntryIds: ["真实 entryId"],
          }],
        },
      }),
  ...
  const thoughtBlock = facts.kind === "monthly-awareness"
    ? {
        thought: {
          ...
          repeatedThemes: [{ summary: "本月反复出现的思想主线", evidenceEntryIds: ["真实 entryId"] }],
          emergingSignals: [{ summary: "本月新出现、但尚未证明稳定的线索", evidenceEntryIds: ["真实 entryId"] }],
          openObservations: [{ question: "有待观察的问题", evidenceEntryIds: ["真实 entryId"] }],
  ...
  return [
    ...
    facts.kind === "historical-baseline"
      ...
      : "这是一次基于新信息的心智模型增量更新。previousModels 与 previousDimensionProfile 是更新前的完整当前画像；thoughts、emotions 与 emotionReviews 是本批新增信息。情绪事件与情绪周报都是心智模型的更新来源。请输出合并更新后的完整当前画像，而不是仅输出本月局部画像。",
    "emotionReviews 中的情绪周报是基于原始事件形成的可能解释，不是新的客观事实。必须与 emotions 的原始记录相互核对；单次情绪或单周推测只能作为线索，不能独自改写整个心智模型。",
    "月度思想分析不得默认同一问题在月初到月末连续演化。repeatedThemes 表示跨记录反复出现的主线；emergingSignals 表示首次或少量出现的新线索；openObservations 用于保留尚未验证的方向。除非输入含有明确的先后证据，否则不得使用“月初如何、月末如何”的叙述。",
    ...
    "所有面向用户的中文必须清晰易懂：使用日常语言，直接说明“怎么看、为什么这样判断、通常会怎么做”；避免抽象术语、理论腔、长定语和重复解释。八个维度必须各自回答本维度的问题，不得复用同一段结论。dimensionProfile.summary 保持 50—80 个汉字，defaultJudgments 与 currentStrategies 每项 12—28 个汉字、各 2—3 项。",
    "输出结构：",
    JSON.stringify({
      kind: facts.kind,
...
function parseStructuredResult(text) {
  ...
  if (value.kind === "weekly-emotion") {
    ...
    return {
      ...
      emotion: {
        ...value.emotion,
        recurringTriggers: Array.isArray(value.emotion.recurringTriggers) ? value.emotion.recurringTriggers : [],
        recoveryPatterns: Array.isArray(value.emotion.recoveryPatterns) ? value.emotion.recoveryPatterns : [],
        notableChanges: Array.isArray(value.emotion.notableChanges) ? value.emotion.notableChanges : [],
        evidenceEntryIds: Array.isArray(value.emotion.evidenceEntryIds) ? value.emotion.evidenceEntryIds : [],
        triggerChains: Array.isArray(value.emotion.triggerChains) ? value.emotion.triggerChains : [],
        alternativeExplanations: Array.isArray(value.emotion.alternativeExplanations) ? value.emotion.alternativeExplanations : [],
        pendingValidations: Array.isArray(value.emotion.pendingValidations) ? value.emotion.pendingValidations : [],
        mentalModelSignals: Array.isArray(value.emotion.mentalModelSignals) ? value.emotion.mentalModelSignals : [],
      },
    };
  }
  ...
  return value.thought && typeof value.thought === "object"
    ? {
        ...value,
        thought: {
          ...value.thought,
          repeatedThemes: Array.isArray(value.thought.repeatedThemes) ? value.thought.repeatedThemes : [],
          emergingSignals: Array.isArray(value.thought.emergingSignals) ? value.thought.emergingSignals : [],
          openObservations: Array.isArray(value.thought.openObservations) ? value.thought.openObservations : [],
        },
      }
    : value;
}
