...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === "/api/runtime" && request.method === "GET") {
      json(response, 200, { runtime: runtime.describe() });
      return;
    }
    if (
      url.pathname.startsWith("/api/")
      || url.pathname.startsWith("/week-up-review-api/")
      || url.pathname.startsWith("/learning-more-api/")
    ) {
    ...
    const unavailable = message === "host_identity_adapter_required";
    const unauthorized = message === "host_identity_invalid";
    json(response, conflict ? 409 : unavailable ? 503 : unauthorized ? 401 : message === "request_too_large" ? 413 : invalidRequest ? 400 : 500, {
      error: message,
